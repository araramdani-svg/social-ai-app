...
const PLANS = [
  ...
  },
  {
    ...
    features: [
      ...
      { text: "Multi-format Repurpose",     included: true  },
      { text: "Priority support",           included: false },
    ],
    cta: "Start Pro →",
    planKey: "pro",
  },
  {
    ...
    features: [
      ...
      { text: "Unlimited analyses",         included: true  },
      { text: "Team Console (5 members)",   included: true  },
      { text: "Multi-format Repurpose",     included: true  },
  ...
  },
  {
    ...
    yearlyPrice: 79,
    badge: "FOR AGENCIES",
    description: "Multi-client content operations",
    color: "#8b5cf6",
    accent: "rgba(139,92,246,0.12)",
    border: "rgba(139,92,246,0.4)",
    features: [
      { text: "Unlimited AI generations",   included: true  },
      { text: "Up to 50 client accounts",   included: true  },
      { text: "Team Console (20 members)",  included: true  },
          ...
          return (
            <div key={plan.key} style={{
              ...
              background: isPro
                ...
                : plan.key === "agency"
                ? "linear-gradient(145deg,#0f0a1a,#130f1a)"
                : "linear-gradient(145deg,#111827,#0f172a)",
              boxShadow: isPro
                ? "0 0 40px rgba(220,38,38,0.15),0 20px 60px rgba(0,0,0,0.4)"
                : plan.key === "agency"
                ? "0 0 40px rgba(139,92,246,0.1),0 20px 60px rgba(0,0,0,0.4)"
              ...
              <div style={styles.features}>
                {plan.features.map((f, i) => (
                  <div key={i} style={styles.feature}>
                    <span style={{ color:f.included?plan.color:"#1e293b", fontSize:14, fontWeight:800 }}>{f.included?"✓":"✗"}</span>
                    <span style={{ color:f.included?"#cbd5e1":"#334155", fontSize:13 }}>{f.text}</span>
                  </div>
                ))}
              </div>
              ...
              <button
                style={{
                  ...
                  background: isActive
                    ...
                    : plan.key === "free"
                    ? "transparent"
                    : `linear-gradient(135deg,${plan.color},${
                        plan.key==="pro"?"#991b1b":
                        plan.key==="business"?"#c2410c":
                        "#7c3aed"
                      })`,
                  border: isActive ? "1px solid #22c55e" : plan.key==="free" ? `1px solid ${plan.border}` : "none",
                  color:  isActive ? "#22c55e" : "#fff",
                  cursor: isActive||plan.key==="free" ? "default" : "pointer",
                  boxShadow: plan.key!=="free"&&!isActive ? `0 4px 20px ${plan.accent}` : "none",
                }}
...
const styles = {
  ...
  navBtn: { padding:"12px 24px", background:"transparent", border:"1px solid rgba(220,38,38,0.25)", borderRadius:10, color:"#ef4444", cursor:"pointer", fontWeight:600, fontSize:13 },
  navCta: { padding:"12px 24px", background:"linear-gradient(135deg,#dc2626,#991b1b)", border:"none", borderRadius:10, color:"white", fontWeight:700, cursor:"pointer", fontSize:13, boxShadow:"0 4px 16px rgba(220,38,38,0.35)" },
  header: { textAlign:"center", marginBottom:56, position:"relative", zIndex:1 },
  badge: { display:"inline-flex", alignItems:"center", gap:8, background:"rgba(220,38,38,0.1)", border:"1px solid rgba(220,38,38,0.3)", borderRadius:20, padding:"6px 16px", fontSize:11, fontWeight:700, color:"#ef4444", letterSpacing:"1.5px", marginBottom:20 },
  title: { fontSize:52, fontWeight:900, lineHeight:1.1, margin:"0 0 16px" },
  titleAccent: { color:"#ef4444" },
  subtitle: { fontSize:16, color:"#475569", margin:"0 0 32px" },
  toggle: { display:"inline-flex", alignItems:"center", gap:12 },
  toggleTrack: { width:48, height:26, background:"rgba(220,38,38,0.3)", border:"1px solid rgba(220,38,38,0.5)", borderRadius:13, cursor:"pointer", position:"relative", transition:"background 0.2s" },
  toggleThumb: { position:"absolute", top:2, width:20, height:20, background:"#ef4444", borderRadius:"50%", transition:"transform 0.2s", boxShadow:"0 2px 8px rgba(220,38,38,0.5)" },
  saveBadge: { marginLeft:8, background:"rgba(220,38,38,0.2)", border:"1px solid rgba(220,38,38,0.4)", borderRadius:6, padding:"2px 6px", fontSize:10, color:"#ef4444", fontWeight:800, letterSpacing:"0.5px" },
  plans: { display:"grid", gap:20, maxWidth:1200, margin:"0 auto", position:"relative", zIndex:1, alignItems:"center" },
  card: { borderRadius:20, padding:"28px 24px", position:"relative", transition:"transform 0.2s" },
  planBadge: { position:"absolute", top:-12, left:"50%", transform:"translateX(-50%)", padding:"4px 14px", borderRadius:20, fontSize:10, fontWeight:800, letterSpacing:"1px", whiteSpace:"nowrap" },
  planHeader: { display:"flex", alignItems:"center", gap:8, marginBottom:6 },
  planDot: { width:8, height:8, borderRadius:"50%" },
  planName: { fontSize:13, fontWeight:800, letterSpacing:"1.5px", textTransform:"uppercase" },
  planDesc: { fontSize:12, color:"#475569", margin:"0 0 20px" },
  priceRow: { display:"flex", alignItems:"flex-end", gap:4, marginBottom:4 },
  currency: { fontSize:20, fontWeight:800, color:"#94a3b8", marginBottom:6 },
  price: { fontSize:52, fontWeight:900, lineHeight:1, color:"#fff" },
  per: { fontSize:14, color:"#475569", marginBottom:8 },
  billedYearly: { fontSize:11, color:"#475569", marginBottom:4 },
  divider: { height:1, margin:"20px 0", opacity:0.3 },
  features: { display:"flex", flexDirection:"column", gap:10, marginBottom:24 },
  feature: { display:"flex", alignItems:"center", gap:10 },
  cta: { width:"100%", padding:"14px", borderRadius:12, fontWeight:800, fontSize:14, letterSpacing:"0.5px", transition:"opacity 0.2s,transform 0.1s", cursor:"pointer" },
  footerNote: { textAlign:"center", color:"#334155", fontSize:13, marginTop:40, position:"relative", zIndex:1 },
};
