...
// ─── Dropdown Teams ───────────────────────────────────────────────────────────
function TeamDropdown({ teamsList, filterTeam, setFilterTeam, onSelect }) {
    ...
    <div style={{ position:"relative" }}>
      ...
      {open && (
        <>
          ...
          <div style={{ position:"absolute", top:"calc(100% + 6px)", left:0, zIndex:101, background:"#0f172a", border:"1px solid rgba(255,255,255,0.1)", borderRadius:12, minWidth:200, overflow:"hidden", boxShadow:"0 16px 40px rgba(0,0,0,0.5)" }}>
            {[{ id:"", name:"🏷️ Toutes les teams" }, ...teamsList].map(t => {
              const isSelected = String(t.id) === String(filterTeam);
              return (
                <button key={t.id ?? "all"}
                  onClick={() => { setFilterTeam(t.id); onSelect(t.id); setOpen(false); }}
        ...
        <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
          ...
          <tbody>
            ...
            {logs.map(log => {
              ...
              const cfg = isChat
                ? (USER_ACTION_LABELS[log.action] || { label:"💬 Chat", color:"#60a5fa" })
                : (ACTION_LABELS[log.action] || { label:log.action, color:"#94a3b8" });
              const userDisplay = isChat
                ? (log.user_email || `#${log.user_id}`)
                : (log.target_email || `#${log.target_user_id}`);
              const teamDisplay = isChat ? log.user_team_name : log.team_name;

              let detailsNode;
              if (isChat) {
                detailsNode = log.chat_content
                  ? <div>
              ...
              return (
                ...
                >
                  ...
                  <td style={{ padding:"12px 16px", fontSize:11 }}>
                    {teamDisplay
                      ? <span style={{ background:"rgba(139,92,246,0.1)", border:"1px solid rgba(139,92,246,0.25)", borderRadius:8, padding:"2px 8px", color:"#a78bfa", fontSize:10, fontWeight:700 }}>{teamDisplay}</span>
                      : <span style={{ color:"#334155" }}>—</span>}
                  </td>
