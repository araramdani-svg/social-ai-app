  ...
  useEffect(() => {
    if (token && token !== "guest") {
      ...
      fetch(`${API}/auth/me`, { headers:{ Authorization:`Bearer ${token}` } })
        .then(r => r.json())
        .then(d => { if (d.onboarding_done === false) setShowOnboarding(true); if (d.first_name) setFirstName(d.first_name); if (d.last_name) setLastName(d.last_name); if (d.display_name) setDisplayName(d.display_name); if (d.plan_managed_by) setPlanManagedBy(d.plan_managed_by); })
        .catch(() => {});
