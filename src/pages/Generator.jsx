...
export default function Generator({ token: tokenProp, trendsLang: langProp, setTrendsLang: setLangProp, setPage }) {
  ...
  useEffect(() => {
    if (token && token !== "guest") {
      ...
      fetch(`${API}/auth/me`, { headers:{ Authorization:`Bearer ${token}` } })
        .then(r => r.json())
        .then(d => { if (d.onboarding_done === false) setShowOnboarding(true); if (d.first_name) setFirstName(d.first_name); if (d.last_name) setLastName(d.last_name); })
        .catch(() => {});
  ...
  const savePost = async () => { if(!post){ showToast("⚠️ No content to save — generate a post first"); return; } const title=projectTitle||selectedProject||topic||"Untitled"; try{ const d=await api("auth/save-post",{ title,content:post,project_name:selectedProject||null }); if(d?.id) setCurrentPostDbId(d.id); setSaveStatus("✅ " + tr(trendsLang,"messages.projectSaved")); showToast(tr(trendsLang,"messages.projectSaved")); setTimeout(()=>setSaveStatus(""),2000); if(selectedProject) { const posts=await fetch(`${API}/auth/posts/by-project/${encodeURIComponent(selectedProject)}`,{headers:{Authorization:`Bearer ${token}`}}).then(r=>r.json()); if(Array.isArray(posts)) setProjectPosts(posts); } } catch{ showToast("❌ Save failed — please try again"); } };
  ...
  const analyze        = async () => { if(!post) return; try{ setLoading(true); const d=await api("analyze",{ text:post, lang:trendsLang }); if(d?.error==="quota_exceeded"){ showToast(`⚠️ ${d.message}`); setTimeout(()=>setPage&&setPage("pricing"),2000); return; } setAnalysis(d); setTab("analyze"); showToast(tr(trendsLang,"messages.analysisComplete")); } catch{ showToast(tr(trendsLang,"messages.analysisFailed")); } finally{ setLoading(false); setAiStep(0); } };
