...
router.get("/chat", auth, async (req, res) => {
  try {
    const teamId = req.query.team_id ? parseInt(req.query.team_id) : await resolveTeamId(req.user.id);
    ...
    const r = await db.query(
      `SELECT tm.id, tm.sender_id, tm.sender_email, tm.sender_name, tm.content, tm.created_at,
       ...
       WHERE tm.team_id=$2 ORDER BY tm.created_at ASC LIMIT $3`,
...
router.post("/chat", auth, async (req, res) => {
  ...
  try {
    ...
    const uRes = await db.query(
      "SELECT email, first_name, last_name, display_name FROM users WHERE id=$1", [req.user.id]
    );
    const u = uRes.rows[0];
